...
function openCreateCatalogDialog($node) {
    ...
    $then.find("input[name='documentId']").val('');
...
function getSiblingSort(node) {
    ...
    for (var key in node.children) {
        ...
        data[index] = {
            "documentId": parseInt(node.children[key]),
            "orderSort": parseInt(key),
            "parentId": Number(node.id) ? Number(node.id) : 0
        };
    }
    return data;
}
...
function openDeleteDocumentDialog($node) {
    console.debug(window.docCUDURL);
...
function openEditCatalogDialog($node) {
    ...
    $then.find("input[name='documentId']").val(doc_id);
    ...
    for (var index in window.documentCategory) {
        ...
        if (item.id === doc_id) {
            $then.find("input[name='identify']").val(item.identify);
            break;
        }
    }
